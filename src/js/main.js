...
function initBlurRevealOnScroll() {
  ...
  // 1. Existing .blur-reveal elements
  document.querySelectorAll('.blur-reveal').forEach((el) => observer.observe(el));

  // 2. Section Headers (Editorial Staggered Rise with Soft Blur Dissolve)
  // Skip Hero sections to guarantee instantaneous LCP performance
  document.querySelectorAll('section:not(.linfopress-hero-stage):not(.zionic-official-hero):not(.hero-section):not(.hero-split-fullscreen-section) .section-header-centered, .presentation-header, .procedure-stage-head, .tech-section-header, .zionic-section-header, .section-header, .bento-header, .clinics-header').forEach((headerWrap) => {
    const title = headerWrap.querySelector('.section-main-title, .presentation-main-head, h2');
    const sub = headerWrap.querySelector('.section-main-sub, .presentation-sub-head, p');
    if (title && !title.classList.contains('t-reveal-header')) {
      title.classList.add('t-reveal-header');
      observer.observe(title);
    }
    if (sub && !sub.classList.contains('t-reveal-header-sub')) {
      sub.classList.add('t-reveal-header-sub');
      observer.observe(sub);
    }
  });

  // 3. Staggered Grids (Bento, Advantages, Results, Packages, Doctor Reviews, Guarantees, FAQs)
  const gridSelectors = [
    '.tech-bento-grid',
    '.linfopress-tech-bento-grid',
    '.linfopress-advantages-col-list',
    '.package-items-grid',
    '.linfopress-package-grid',
    '.results-tiles-grid',
    '.zionic-doctors-grid',
    '.guarantees-stack',
    '.bento-container',
  ...
  gridSelectors.forEach((gridSel) => {
    document.querySelectorAll(gridSel).forEach((grid) => {
      ...
      Array.from(items).forEach((item, index) => {
        if (!item.classList.contains('t-reveal-item')) {
          item.classList.add('t-reveal-item');
          item.style.setProperty('--item-idx', Math.min(index, 8));
  ...
  // 4. Split Matrices (Indications vs Contraindications)
  document.querySelectorAll('.zionic-creative-matrix-grid, .linfopress-indications-creative-grid, .linfopress-split-grid').forEach((matrix) => {
    const left = matrix.querySelector('.indications-card, .matrix-creative-card:first-child');
    const right = matrix.querySelector('.contraindications-card, .matrix-creative-card:last-child');
    if (left && !left.classList.contains('t-reveal-from-left')) {
      left.classList.add('t-reveal-from-left');
      observer.observe(left);
    }
    if (right && !right.classList.contains('t-reveal-from-right')) {
      right.classList.add('t-reveal-from-right');
      observer.observe(right);
    }
  });

  // 5. Cinematic Media Stages (Infographics, Fullscreen Videos, Big Showcase Images)
  document.querySelectorAll('.treatments-infographic-viewport, .zionic-video-fullscreen-banner, .cinema-video-box, .presentation-photo-frame, .expert-visual-box, .expert-photo-wrapper').forEach((media) => {
    if (!media.classList.contains('t-reveal-scale')) {
      media.classList.add('t-reveal-scale');
      observer.observe(media);
    }
  });

  // 6. Application Stage Columns
  document.querySelectorAll('.presentation-visual-col').forEach((col) => {
    if (!col.classList.contains('t-reveal-from-left')) {
      col.classList.add('t-reveal-from-left');
      observer.observe(col);
    }
  });
  document.querySelectorAll('.presentation-form-col, .presentation-form-card').forEach((form) => {
    if (!form.classList.contains('t-reveal-from-right')) {
      form.classList.add('t-reveal-from-right');
