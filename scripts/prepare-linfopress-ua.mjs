...
let body = fs.readFileSync('C:/nextweb/termosalud/scripts/termosalud_ua_body.html', 'utf8');

// Replace all external domain occurrences with local paths
body = body.replace(/https:\/\/termosalud\.com\.ua/g, '');

// Escape any backticks or ${ in body so template literal does not break
body = body.replace(/`/g, '\\`').replace(/\${/g, '\\${');

// Read clean process-html.mjs template
...
const linfopressBlock = `  // Modern Linfopress Evolution PRO Page Structure (Cloned from termosalud.com.ua/linfopress/ with Custom Video Hero)
