...
`;
  ...
  const typographyStudioHtml = `
    <!-- Minimalist Typography & Screen Heading Studio Widget -->
    <div id="typography-studio-widget" class="typography-studio-widget">
      
      <!-- Floating Minimal Trigger Button -->
      <button id="studio-trigger-btn" class="typography-studio-btn" title="Налаштування шрифтів та товщини заголовків">
        ...
        <span>Типографіка екранів</span>
        <svg class="chevron-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg>
      </button>

      <!-- Floating Dropdown Control Panel -->
      <div id="studio-panel" class="typography-studio-panel">
        ...
        <div class="studio-panel-header">
          <div class="studio-panel-title">
            ...
            <span>Студія заголовків</span>
          </div>
          <button id="studio-close-btn" class="studio-panel-close" title="Закрити меню">✕</button>
        </div>

        <!-- Control 1: Font Family -->
        <div class="studio-field-group">
          <label class="studio-field-label" for="studio-font-select">
            <span>Шрифт сайту</span>
          </label>
          <select id="studio-font-select" class="studio-select">
            <option value="montserrat">Montserrat (Оригінал бренду)</option>
            <option value="manrope">Manrope (Swiss Tech / Мінімалізм)</option>
            <option value="jakarta">Plus Jakarta Sans (Modern Clean)</option>
            <option value="inter">Inter (Pixel Precision)</option>
            <option value="onest">Onest (Кирилиця Pro)</option>
            <option value="outfit">Outfit (Aesthetic)</option>
            <option value="golos">Golos Text (Editorial)</option>
            <option value="playfair">Playfair Display (Fashion Luxe)</option>
          </select>
        </div>

        <!-- Control 2: Target Screen -->
        <div class="studio-field-group">
          <label class="studio-field-label" for="studio-screen-select">
            <span>Оберіть екран для тесту</span>
          </label>
          <select id="studio-screen-select" class="studio-select">
            <option value="all">🌐 Усі екрани (Глобально)</option>
            <option value="s1">1️⃣ Екран 1: Головний (Hero)</option>
            <option value="s2">2️⃣ Екран 2: Обладнання (Zionic / Linfopress)</option>
            <option value="s3">3️⃣ Екран 3: Чому Termosalud (Переваги)</option>
            <option value="s4">4️⃣ Екран 4: Презентація (Тест-драйв)</option>
            <option value="s5">5️⃣ Екран 5: Партнери (Клініки)</option>
            <option value="s6">6️⃣ Екран 6: Експертиза (SEO / Стаття)</option>
          </select>
        </div>

        <!-- Control 3: Weight Selector -->
        <div class="studio-field-group">
          <div class="studio-field-label">
            <span>Товщина заголовка</span>
            <span id="studio-weight-label" class="highlight-val">700 Bold</span>
          </div>
          <div class="studio-weight-pills">
            <button type="button" class="studio-weight-btn" data-weight="300">300</button>
            <button type="button" class="studio-weight-btn" data-weight="400">400</button>
            <button type="button" class="studio-weight-btn" data-weight="500">500</button>
            <button type="button" class="studio-weight-btn" data-weight="600">600</button>
            <button type="button" class="studio-weight-btn active" data-weight="700">700</button>
            <button type="button" class="studio-weight-btn" data-weight="800">800</button>
            <button type="button" class="studio-weight-btn" data-weight="900">900</button>
          </div>
        </div>
        ...
        <div class="studio-field-group">
          <div class="studio-field-label">
            <span>Готові пресети</span>
          </div>
          <div class="studio-presets-row">
            <button type="button" id="preset-authentic" class="studio-preset-btn" title="Hero 900, Інші 700">⭐ Оригінал</button>
            <button type="button" id="preset-luxury" class="studio-preset-btn" title="Hero 700, Інші 500/600">✨ Витончений</button>
            <button type="button" id="preset-heavy" class="studio-preset-btn" title="Всі 900">⚡ Bold Tech</button>
          </div>
        </div>
    ...
    <script>
      (function() {
        ...
        const widget = document.getElementById('typography-studio-widget');
        const triggerBtn = document.getElementById('studio-trigger-btn');
        const closeBtn = document.getElementById('studio-close-btn');
        const fontSelect = document.getElementById('studio-font-select');
        const screenSelect = document.getElementById('studio-screen-select');
        const weightBtns = document.querySelectorAll('.studio-weight-btn');
        const weightLabel = document.getElementById('studio-weight-label');

        const presetAuth = document.getElementById('preset-authentic');
        const presetLux = document.getElementById('preset-luxury');
        const presetHeavy = document.getElementById('preset-heavy');
        ...
        const screenTargets = {
          's1': '.hero-clean-section',
          's2': '#our-products',
          's3': '.why-us-bento-section',
          's4': '#presentation-section',
          's5': '.partners',
          's6': '.seo_text'
        };
        ...
        // State
        let state = {
          font: localStorage.getItem('ts_font') || 'montserrat',
          targetScreen: 'all',
          weights: JSON.parse(localStorage.getItem('ts_weights') || '{"s1":900,"s2":700,"s3":700,"s4":700,"s5":700,"s6":700}')
        };

        function applyTypography() {
          const family = fontFamilies[state.font] || fontFamilies['montserrat'];
          root.style.setProperty('--active-font', family);
          root.style.setProperty('--active-heading-font', family);

          ['s1','s2','s3','s4','s5','s6'].forEach(function(s) {
            const w = state.weights[s] || 700;
            root.style.setProperty('--' + s + '-title-weight', w);
          });

          localStorage.setItem('ts_font', state.font);
          localStorage.setItem('ts_weights', JSON.stringify(state.weights));
        ...
        function updateControls() {
          ...
          if (screenSelect) screenSelect.value = state.targetScreen;

          let curWeight = 700;
          if (state.targetScreen === 'all') {
            curWeight = state.weights['s1'] || 900;
          } else {
            curWeight = state.weights[state.targetScreen] || 700;
          }

          if (weightLabel) weightLabel.textContent = weightNames[curWeight] || curWeight;

          weightBtns.forEach(function(b) {
            if (parseInt(b.getAttribute('data-weight'), 10) === parseInt(curWeight, 10)) {
        ...
        // Screen target change + Smooth scroll to section
        if (screenSelect) {
          screenSelect.addEventListener('change', function() {
            state.targetScreen = screenSelect.value;
            updateControls();

            if (state.targetScreen !== 'all' && screenTargets[state.targetScreen]) {
              const el = document.querySelector(screenTargets[state.targetScreen]);
              if (el) {
        ...
        weightBtns.forEach(function(b) {
          b.addEventListener('click', function() {
            const w = parseInt(b.getAttribute('data-weight'), 10);
            if (state.targetScreen === 'all') {
              ['s1','s2','s3','s4','s5','s6'].forEach(function(s) {
                state.weights[s] = w;
              });
            } else {
              state.weights[state.targetScreen] = w;
            }
            applyTypography();
          });
        });
        ...
        // Presets
        if (presetAuth) {
          presetAuth.addEventListener('click', function() {
            state.weights = { s1: 900, s2: 700, s3: 700, s4: 700, s5: 700, s6: 700 };
            applyTypography();
          });
        }

        if (presetLux) {
          presetLux.addEventListener('click', function() {
            state.weights = { s1: 700, s2: 500, s3: 600, s4: 600, s5: 600, s6: 500 };
            applyTypography();
          });
        }

        if (presetHeavy) {
          presetHeavy.addEventListener('click', function() {
            state.weights = { s1: 900, s2: 900, s3: 900, s4: 900, s5: 900, s6: 900 };
            applyTypography();
          });
        }
