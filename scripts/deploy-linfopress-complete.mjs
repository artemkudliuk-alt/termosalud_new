...
const lpStartIdx = htmlMjs.indexOf("if (pageName === 'linfopress') {");
const lpEndIdx = htmlMjs.indexOf("const modernLuxuryFooter", lpStartIdx);
...
if (lpStartIdx > -1 && lpEndIdx > -1) {
  ...
  htmlMjs = htmlMjs.substring(0, lpStartIdx) + replacement + htmlMjs.substring(lpEndIdx + "const modernLuxuryFooter".length);
  fs.writeFileSync('C:/nextweb/termosalud/scripts/process-html.mjs', htmlMjs, 'utf8');
  console.log('Successfully written new Linfopress generation template to scripts/process-html.mjs!');
} else {
  console.error('Could not slice scripts/process-html.mjs properly!');
}
